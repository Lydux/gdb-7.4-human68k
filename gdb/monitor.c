...
static int
monitor_write_memory_words (CORE_ADDR memaddr, char *myaddr, int len)
{
  enum bfd_endian byte_order = gdbarch_byte_order (target_gdbarch);
  gdb_byte val[2];
  int written = 0;
  ...
  /* Enter the sub mode.  */
  monitor_printf (current_monitor->setmem.cmdw, memaddr);
  ...
  while (len)
    {
      ...
      store_unsigned_integer (val, 2, byte_order, *(short *) myaddr);

      monitor_printf ("%04x\r", *(short *) val);
      myaddr += 2;
      written += 2;
      len -= 2;
    }
  ...
  /* Now exit the sub mode.  */
  monitor_printf (current_monitor->setmem.term_cmd);
...
#endif
  if (current_monitor->setmem.cmdb)
    written = monitor_write_memory_bytes (memaddr, myaddr, len);
  else if (current_monitor->setmem.cmdw)
    written = monitor_write_memory_words (memaddr, myaddr, len);
...
monitor_read_memory (CORE_ADDR memaddr, char *myaddr, int len)
{
  ...
  if (current_monitor->flags & MO_GETMEM_NEEDS_RANGE)
    {
      if (current_monitor->flags & MO_ADDR_RANGE_INCLUSIVE)
  ...
  while (i > 0)
	...
	monitor_error ("monitor_read_memory",
      ...
      store_unsigned_integer (myaddr, val_size, byte_order, val);
